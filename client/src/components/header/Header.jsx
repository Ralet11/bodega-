      ...
      <nav className='flex items-center justify-between p-1'>
        <div className='md:hidden flex items-center mr-2 md:pl-2'>
          <img
            ...
            className={isDistributorsCommerce ? 'w-4 py-2' : ""}
            alt='Logo'
          />
        </div>
        ...
        {isDistributorsCommerce && (
          <div className='flex bg-white items-center justify-center shadow-lg rounded-full overflow-hidden flex-grow mx-1'>
            <input
              type='text'
              value={searchQuery}
              ...
              className='w-full p-1 text-xs sm:text-sm border-none focus:outline-none'
              style={{
            ...
            />
            <button className='m-1 p-1 bg-gray-100 rounded-full hover:bg-gray-200 transition duration-300'>
              <svg
        ...
        <div className='flex items-center space-x-2'>
          <CartIcon />
          {isDistributorsCommerce && (
            <LiaCartPlusSolid className='w-4 h-4 hidden' />
          )}

          <Bars3Icon ref={barsRef} className='w-5 h-5 text-gray-700 block sm:hidden' onClick={toggleDropdown} />
          <div className='relative hidden sm:flex items-center'>
            <span
              ref={triggerRef}
              onClick={toggleDropdown}
              className='p-1 font-bold text-xs sm:text-sm hover:text-indigo-500 rounded-xl cursor-pointer flex items-center bg-[#F2BB26] text-gray-700 hover:text-gray-900'
            >
            ...
            {dropdownOpen && (
              <ul
                ref={dropdownRef}
                className='absolute text-xs sm:text-sm right-0 md:right-6 top-full mt-2 w-40 bg-white border border-gray-200 rounded-md shadow-md z-20'
              >
                <li>
                  <Link
          ...
          {dropdownOpen && (
            <div className='relative sm:hidden'>
              <ul
                ref={dropdownRef}
                className='absolute text-xs sm:text-sm right-0 top-full mt-2 w-40 bg-white border border-gray-200 rounded-md shadow-md z-20'
              >
                <li>
                  <Link
                    className='block px-3 py-2 text-gray-700 hover:bg-gray-100'
